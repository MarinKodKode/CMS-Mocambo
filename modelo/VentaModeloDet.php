...
class VentaModeloDet
{
    var $objetos_venta_modelo_det;
    ...
    function ver($id)
    {
        $sql = "SELECT * FROM venta_producto_det JOIN producto on producto.id_producto = venta_producto_det.vp_id_producto where venta_id_venta=:id";
        $query = $this->access->prepare($sql);
        $query->execute(array(':id' => $id));
        $this->objetos_venta_modelo_det = $query->fetchall();
        return $this->objetos_venta_modelo_det;
    }
    /*function totalesProducto($id_ruta, $fecha)
    {
        ...
        $this->objetos_venta_modelo = $query->fetchall();
        return $this->objetos_venta_modelo;
    }*/
}
